...
char* cena_main = "John Cena:d=4,o=5,b=128:8p,8a.,8b,8g,16p,2a,16p,8p,8c.6,8b,8g,16p,2a,16p";
...
/// FUNCTIONS ///
/////////////////
void play_rtttl(uint8_t pin, char* p)
{
...
void play_sound(char* file_name) {
  if (SOUNDS_ON) {
    enable_audio();
    play_rtttl(SPEAKER_PIN, file_name);
    disable_audio();
  }
}
...
void init_audio_core(void) {
  ...
  #ifdef ENABLE_SOUNDS
    SOUNDS_ON = true;
    //play_rtttl(SPEAKER_PIN, cena_main);
  #else
    SOUNDS_ON = false;
  #endif
  PRINTLN("done");
